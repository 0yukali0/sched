...
func (sched *Scheduler) ScheduleBab() {
	...
	nodes := sched.config.NodeLister.List()
	...
	for index, pod := range pods {
		filterdNodes, _, _ := sched.config.Algorithm.PubToPredict(pod, nodes)
	...
	for _, node := range nodeBook {
		...
		for _, pod := range chosenPods {
			...
			allBound, err := sched.assumeVolumes(assumedPod, node.Name)
			if err != nil {
			...
			for _, pl := range plugins.ReservePlugins() {
				if err := pl.Reserve(plugins, assumedPod, node.Name); err != nil {
			...
			err = sched.assume(assumedPod, node.Name)
			if err != nil {
			...
			go func() {
				...
				for _, pl := range plugins.PrebindPlugins() {
					approved, err := pl.Prebind(plugins, assumedPod, node.Name)
					if err != nil {
						approved = false
